...
import {
  ...
  MapPin, Phone, Mail, CreditCard, AlertCircle,
} from "lucide-react";
...
function StatusBadge({ status }: { status: string }) {
  return (
    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold capitalize ${STATUS_COLORS[status] ?? "bg-gray-100 text-gray-700"}`}>
      {status}
    </span>
  );
}
      ...
      <div className="bg-white w-full sm:max-w-2xl sm:rounded-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#f0ece4] sticky top-0 bg-white z-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={onClose} className="p-2 hover:bg-[#f0f4ed] rounded-lg">
              <X size={18} className="text-[#5a8a50]" />
            </button>
          </div>
        </div>
        ...
        <div className="p-5 flex flex-col gap-5">
          ...
          {next.length > 0 && (
            <div>
              <p className="text-xs font-bold text-[#1e3d18] uppercase tracking-wide mb-2">
                Update Status
              </p>
              ...
              {(order.status === "processing" || order.status === "confirmed") && order.customerEmail && (
                <p className="text-xs text-[#7a9e6a] mt-2 flex items-center gap-1">
                  <Mail size={11} />
                  Customer will receive an email when marked as "shipped" or "delivered"
                </p>
              )}
              {!order.customerEmail && (
                <p className="text-xs text-amber-600 mt-2 flex items-center gap-1">
                  <AlertCircle size={11} />
                  No email on record — status update email cannot be sent
                </p>
              )}
            </div>
          )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-[#f8faf6] rounded-xl p-4">
              <p className="text-xs font-bold text-[#1e3d18] uppercase tracking-wide mb-2 flex items-center gap-1.5">
                <Phone size={12} />Customer
              </p>
              <p className="text-sm font-bold text-[#2a2a1e]">{order.customerName}</p>
              <a
                href={`tel:${order.customerPhone}`}
                className="text-sm text-[#3d6b35] font-semibold hover:underline block"
              >
                {order.customerPhone}
              </a>
              {order.customerEmail && (
                <a
                  ...
                  className="text-xs text-[#3d6b35] hover:underline flex items-center gap-1 mt-1"
                >
                  <Mail size={11} />
                  {order.customerEmail}
                </a>
          ...
          {/* Items */}
          <div>
            <p className="text-xs font-bold text-[#1e3d18] uppercase tracking-wide mb-3">
            ...
            <div className="bg-white border border-[#dce8d4] rounded-xl overflow-hidden">
              {(order.items ?? []).map((item, i) => (
                <div
                  key={i}
                  className="flex items-center gap-3 px-4 py-3 border-b border-[#f0f4ed] last:border-0"
                >
                  {item.image && (
                    <div className="relative w-11 h-11 rounded-lg overflow-hidden bg-[#f5f0ea] shrink-0">
              ...
              <div className="px-4 py-3 bg-[#f8faf6] flex flex-col gap-1.5">
                ...
                <div className="flex justify-between items-baseline pt-1.5 border-t border-[#dce8d4]">
                  ...
                  <span className="text-base font-black text-[#3d6b35]">
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-[#1e3d18] rounded-xl px-5 py-4 flex items-center justify-between">
        <div>
          <p className="text-xs text-[#7ab648] font-bold uppercase tracking-wide">Total Pipeline Revenue</p>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-[#f8faf6] border-b border-[#dce8d4]">
                {["Order #", "Customer", "Items", "Total", "Status", "Date", ""].map((h) => (
                  <th
                    key={h}
                    className="px-4 py-3 text-left text-xs font-bold text-[#5a8a50] uppercase tracking-wider whitespace-nowrap"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="px-4 py-10 text-center text-sm text-[#9ab890]">
                    No orders found.
                  </td>
                </tr>
              ) : (
                filtered.map((order) => (
                  <tr
                    key={order._id}
                    className="border-b border-[#f0f4ed] hover:bg-[#fafcf8] transition-colors"
                  >
                    <td className="px-4 py-3 text-xs font-mono font-bold text-[#1e3d18] whitespace-nowrap">
                      {order.orderNumber}
                    </td>
                    <td className="px-4 py-3">
                      <p className="text-sm font-semibold text-[#2a2a1e] truncate max-w-[140px]">
                        {order.customerName}
                      </p>
                      <p className="text-xs text-[#9ab890]">{order.customerPhone}</p>
                      {order.customerEmail && (
                        <p className="text-xs text-[#7a9e6a] truncate max-w-[140px]">
                          {order.customerEmail}
                        </p>
                      )}
                    </td>
                    <td className="px-4 py-3 text-xs text-[#5a8a50]">
                      {order.items?.length ?? 0} item{(order.items?.length ?? 0) !== 1 ? "s" : ""}
                    </td>
                    <td className="px-4 py-3 text-sm font-bold text-[#3d6b35] whitespace-nowrap">
                      ₹{order.total?.toLocaleString("en-IN")}
                    </td>
                    <td className="px-4 py-3">
                      <StatusBadge status={order.status} />
                    ...
                    <td className="px-4 py-3">
                      <button
                        ...
                        className="p-2 hover:bg-[#eef7e6] text-[#3d6b35] rounded-lg transition-colors"
                        title="View"
                      >
                        <Eye size={16} />
                      </button>
