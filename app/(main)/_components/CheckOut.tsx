    ...
    <div className="flex flex-col gap-6">
      ...
      {savedAddresses.map((addr, i) => (
        ...
        >
          ...
          {selected === i && (
            <button className="shrink-0 flex items-center gap-1 text-xs font-semibold text-[#3d6b35] hover:underline mt-0.5">
              <Pencil size={12} /> Edit
            </button>
          )}
        </button>
      ...
      <div className="bg-white border border-[#e8e0d0] rounded-2xl overflow-hidden">
          ...
          {cartItems.map((item) => (
            <div key={item.id} className="flex items-center gap-4 px-5 py-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-[#7a7a68]">{item.variant}</p>
              </div>
              <p className="text-lg font-black text-[#3d6b35] shrink-0">₹{item.price.toLocaleString("en-IN")}</p>
            </div>
          ))}
        </div>
        <div className="px-5 py-4 bg-[#faf7f2] border-t border-[#e8e0d0] flex flex-col gap-2">
          ...
          <div className="flex justify-between text-sm text-[#3d6b35]">
            <span>Coupon discount</span><span className="font-semibold">−₹{couponDiscount}</span>
          </div>
          <div className="flex justify-between text-sm text-[#3d6b35]">
            <span>Delivery</span><span className="font-semibold">FREE</span>
          </div>
              ...
              <div className="flex flex-col divide-y divide-[#f0ece4]">
                {cartItems.map((item) => (
                  <div key={item.id} className="flex items-center gap-3 px-5 py-3.5">
                    ...
                    <p className="text-sm font-bold text-[#3d6b35] shrink-0">₹{item.price}</p>
                  </div>
                ))}
              </div>
