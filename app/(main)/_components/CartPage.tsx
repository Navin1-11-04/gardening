...
import {
  Trash2, Plus, Minus, ChevronRight, Tag,
  Truck, ShieldCheck, Phone, ShoppingBag, ArrowRight, X,
} from "lucide-react";
...
const CartItemRow = ({
  ...
  <div className="flex flex-row items-start gap-3 sm:gap-4 bg-white rounded-2xl border border-[#e8e0d0] p-3 sm:p-4 hover:border-[#b8d4a0] transition-colors">
    {/* Product image */}
    <Link
      href={`/shop/product/${item.id}`}
      className="relative shrink-0 w-20 h-20 sm:w-24 sm:h-24 rounded-xl overflow-hidden bg-[#f5f0ea] border border-[#e8e0d0]"
    >
      <Image src={item.image} alt={item.name} fill className="object-cover" />
      {item.badge && (
        <span className="absolute top-1 left-1 bg-[#3d6b35] text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full leading-tight">
          {item.badge}
        </span>
      )}
    </Link>
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <Link href={`/shop/product/${item.id}`}>
            <h3 className="text-sm sm:text-base font-bold text-[#2a2a1e] leading-snug hover:text-[#3d6b35] transition-colors line-clamp-2">
              {item.name}
            </h3>
          </Link>
          <p className="text-xs text-[#7a7a68] mt-0.5 line-clamp-1">{item.subtitle}</p>
          <span className="inline-block mt-1 text-xs font-semibold text-[#5a5a48] bg-[#f0ece4] px-2 py-0.5 rounded-full">
            {item.variant}
          </span>
        </div>
        {/* Price top-right */}
        <div className="shrink-0 text-right">
          <p className="text-base sm:text-lg font-black text-[#3d6b35]">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center border-2 border-[#d4c9a8] rounded-xl overflow-hidden bg-[#faf7f2]">
          <button
            ...
            className="w-9 h-9 flex items-center justify-center hover:bg-[#eef5ea] transition-colors active:scale-95"
          >
            <Minus size={15} className="text-[#3d6b35]" />
          </button>
          ...
          <button
            ...
            className="w-9 h-9 flex items-center justify-center hover:bg-[#eef5ea] transition-colors active:scale-95"
          >
            <Plus size={15} className="text-[#3d6b35]" />
          </button>
        </div>

        <button
          onClick={() => onRemove(item.id)}
          className="flex items-center gap-1.5 text-xs text-[#c0392b] hover:bg-red-50 px-2.5 py-1.5 rounded-lg transition-colors"
        >
          <Trash2 size={13} />
    ...
    <div className="min-h-screen bg-[#faf7f2]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-start">
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-3">
            ...
            <div className="bg-white rounded-2xl border border-[#e8e0d0] p-4 sm:p-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-base font-bold text-[#2a2a1e]">Have a Coupon?</h3>
              </div>

              {appliedCoupon ? (
                <div className="flex items-center justify-between bg-[#eef5ea] border border-[#b8d4a0] rounded-xl px-4 py-3">
                  <div>
                    <p className="text-sm font-bold text-[#3d6b35]">"{appliedCoupon.code}" applied!</p>
                    <p className="text-xs text-[#5a5a48]">{appliedCoupon.discount}% off — saving ₹{couponDiscount}</p>
                  </div>
                  <button onClick={handleRemoveCoupon} className="p-1.5 rounded-lg hover:bg-white transition-colors">
                    <X size={16} className="text-[#7a7a68]" />
                  </button>
                </div>
              ) : (
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-[#faf7f2] border-2 border-[#d4c9a8] rounded-xl px-3 py-2.5 text-sm text-[#2a2a1e] placeholder:text-[#b0a890] outline-none focus:border-[#3d6b35] transition-colors font-medium"
                  />
                  <button
                    onClick={handleApplyCoupon}
                    className="bg-[#3d6b35] hover:bg-[#335c2c] text-white font-bold text-sm px-4 py-2.5 rounded-xl transition-colors whitespace-nowrap"
                  >
                    Apply
                  </button>
                </div>
              ...
              {couponError && <p className="text-xs text-[#c0392b] mt-1.5 font-medium">{couponError}</p>}
              {!appliedCoupon && (
                <p className="text-xs text-[#a8a090] mt-1.5">Try: GARDEN10, KAVIN20, or GREEN15</p>
              )}
            </div>
          ...
          <div className="w-full lg:w-80 shrink-0 flex flex-col gap-4 lg:sticky lg:top-24">
              ...
              <div className="p-4 sm:p-5 flex flex-col gap-3">
                ...
                <div className="border-t-2 border-[#e8e0d0] pt-3 mt-1">
                  ...
                  {savings + couponDiscount > 0 && (
                    <p className="text-xs text-[#3d6b35] font-semibold mt-1 text-right">
                ...
                <Link
                  href="/checkout"
                  className="mt-1 w-full flex items-center justify-center gap-2 bg-[#3d6b35] hover:bg-[#2e5228] text-white font-bold text-base py-4 rounded-xl transition-all duration-200 active:scale-[.98] shadow-md"
                >
                  Proceed to Checkout
                  <ArrowRight size={20} />
                </Link>
                ...
                <div className="flex items-center justify-center gap-3 pt-1 flex-wrap">
                  <span className="flex items-center gap-1 text-xs text-[#7a7a68]">
                    <ShieldCheck size={13} className="text-[#3d6b35]" />
                    Secure checkout
                  </span>
                  <span className="text-[#d4c9a8]">|</span>
                  <span className="flex items-center gap-1 text-xs text-[#7a7a68]">
                    <Truck size={13} className="text-[#3d6b35]" />
                ...
                <div className="flex gap-1.5 justify-center mt-1 flex-wrap">
                  {["VISA", "Mastercard", "UPI", "G Pay", "Net Banking"].map((p) => (
                    <span key={p} className="border border-[#e8e0d0] px-2 py-0.5 rounded-lg text-[10px] text-[#7a7a68] font-medium">
                      {p}
                    </span>
                  ))}
                </div>
            ...
            <div className="bg-[#faf7f2] border border-[#d4c9a8] rounded-2xl px-4 py-4 flex items-start gap-3">
              <Phone size={18} className="text-[#3d6b35] shrink-0 mt-0.5" />
              <div>
                <p className="text-sm font-bold text-[#2a2a1e]">Need help with your order?</p>
                <a href="tel:+919876543210" className="inline-block mt-1 text-sm font-bold text-[#3d6b35] hover:underline">
                  📞 +91 98765 43210
                </a>
              </div>
            </div>
      ...
      {removedItem && (
        <div className="fixed bottom-24 lg:bottom-6 left-1/2 -translate-x-1/2 z-50 bg-[#2a2a1e] text-white rounded-2xl px-5 py-3.5 flex items-center gap-4 shadow-2xl max-w-sm w-[calc(100%-2rem)]">
          ...
          <button
            onClick={() => { setRemovedItem(null); setUndoBuffer(null); }}
