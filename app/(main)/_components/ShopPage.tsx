...
import { ShoppingCart, SlidersHorizontal, X, ChevronDown, ChevronUp, Star, Loader2 } from "lucide-react";
...
const FilterSidebar = ({
  ...
  onlyOrganic, setOnlyOrganic,
  onClear,
}: {
  priceRange: [number, number]; setPriceRange: (v: [number, number]) => void;
  onlyOnSale: boolean; setOnlyOnSale: (v: boolean) => void;
  onlyOrganic: boolean; setOnlyOrganic: (v: boolean) => void;
  onClear: () => void;
}) => {
  ...
  return (
    <aside className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={onClear} className="text-sm text-[#3d6b35] font-semibold hover:underline">Clear all</button>
      </div>
      ...
      <div className="bg-white border border-[#e8e0d0] rounded-2xl overflow-hidden">
        ...
        >
          Price Range {priceOpen ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
        </button>
        {priceOpen && (
          <div className="px-4 pb-5 flex flex-col gap-4">
            ...
            <input type="range" min={0} max={800} step={10} value={priceRange[1]}
      ...
      <div className="bg-white border border-[#e8e0d0] rounded-2xl overflow-hidden">
        <button onClick={() => setFilterOpen(!filterOpen)}
          className="w-full flex items-center justify-between px-4 py-4 text-base font-bold text-[#2a2a1e]"
        >
          Quick Filters {filterOpen ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
        </button>
        {filterOpen && (
      ...
      <div className="bg-[#eef5ea] border border-[#b8d4a0] rounded-2xl p-4">
        ...
        <p className="text-xs text-[#5a5a48] mb-3">Our garden experts can help you choose.</p>
  ...
  return (
    <div className="min-h-screen bg-[#faf7f2] font-sans">
      ...
      <div className="relative w-full overflow-hidden" style={{ height: "clamp(180px, 28vw, 320px)" }}>
        <img src={heroBg[activeCategory] ?? heroBg.all} alt={heroLabel} className="w-full h-full object-cover transition-all duration-700" />
        ...
        <div className="absolute inset-0 flex flex-col justify-end pb-8 sm:pb-12">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 w-full">
            <p className="text-white/70 text-sm font-medium mb-1">Shop / <span className="text-white">{heroLabel}</span></p>
            <h1 className="text-white font-black font-outfit leading-none" style={{ fontSize: "clamp(1.8rem, 5vw, 3.5rem)" }}>
              {heroLabel}
            </h1>
            ...
            {categories.map((cat) => (
              <button key={cat.id} onClick={() => setActiveCategory(cat.id)}
                className={`shrink-0 px-4 sm:px-5 py-3 text-sm sm:text-base font-semibold rounded-xl transition-all whitespace-nowrap ${
                  activeCategory === cat.id ? "bg-[#3d6b35] text-white shadow-sm" : "text-[#5a5a48] hover:bg-[#eef5ea] hover:text-[#3d6b35]"
                }`}
              >
                {cat.label}
              </button>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 rounded-2xl p-6 text-center mb-8">
            <p className="text-base font-bold text-red-700 mb-2">Couldn't load products</p>
        ...
        <div className="flex items-center justify-between mb-6 gap-3 flex-wrap">
          <p className="text-base text-[#5a5a48]">
            {loading
              ? <span className="flex items-center gap-2"><Loader2 size={16} className="animate-spin text-[#3d6b35]" /> Loading…</span>
        ...
        <div className="flex gap-8">
          <div className="hidden lg:block w-64 shrink-0">
            <FilterSidebar
              ...
              onlyOnSale={onlyOnSale} setOnlyOnSale={setOnlyOnSale}
              onlyOrganic={onlyOrganic} setOnlyOrganic={setOnlyOrganic}
          ...
          <div className="flex-1 min-w-0">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <button onClick={clearFilters} className="bg-[#3d6b35] text-white font-bold px-6 py-3 rounded-xl hover:bg-[#335c2c] transition-colors">
                  Clear Filters
                </button>
              </div>
            ) : (
        ...
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-50 bg-[#faf7f2] rounded-t-3xl p-5 max-h-[80dvh] overflow-y-auto shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setMobileFiltersOpen(false)} className="w-10 h-10 rounded-xl bg-white border border-[#e8e0d0] flex items-center justify-center">
