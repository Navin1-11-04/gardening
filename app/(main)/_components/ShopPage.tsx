      ...
      <div className="p-3 sm:p-4 flex flex-col flex-1 gap-1.5">
        ...
        {product.nameTa && (
          <p className="text-base sm:text-lg font-bold text-[#2a2a1e] leading-snug font-outfit">
            <Highlight text={product.nameTa} query={searchQuery} />
          </p>
        )}

        {/* English name */}
        <div>
          <h3 className={`font-semibold leading-snug ${product.nameTa ? "text-xs sm:text-sm text-[#5a5a48]" : "text-sm sm:text-base font-bold text-[#2a2a1e]"}`}>
...
const FilterSidebar = ({
  priceRange, setPriceRange, onlyOnSale, setOnlyOnSale,
  onlyOrganic, setOnlyOrganic, onClear,
}: {
...
export default function ShopPage() {
  ...
  useEffect(() => { if (searchQuery) setActiveCategory("all"); }, [searchQuery]);
  ...
  const filtered = useMemo(() => {
    let result = products.filter((p) => {
      if (searchQuery) {
        ...
        // FIX: search now checks English name, Tamil name, subtitle, Tamil subtitle, description, category
        const matches =
          p.name.toLowerCase().includes(q) ||
          (p.nameTa?.toLowerCase().includes(q) ?? false) ||
          p.subtitle.toLowerCase().includes(q) ||
          (p.subtitleTa?.toLowerCase().includes(q) ?? false) ||
          p.description.toLowerCase().includes(q) ||
          p.category.toLowerCase().includes(q);
        if (!matches) return false;
      }
      if (!searchQuery && activeCategory !== "all" && p.category !== activeCategory) return false;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
        {error && (
          <div className="bg-red-50 border border-red-200 rounded-2xl p-6 text-center mb-8">
            <p className="text-base font-bold text-red-700 mb-2">Could not load products</p>
        ...
        <div className="flex items-center justify-between mb-6 gap-3 flex-wrap">
          <p className="text-base text-[#5a5a48]">
            {loading ? (
              ...
              <>
                Showing{" "}
                <span className="font-bold text-[#2a2a1e]">{filtered.length}</span>{" "}
