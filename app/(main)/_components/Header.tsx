    ...
    <header className="w-full bg-[#faf7f2] sticky top-0 z-50 border-b-2 border-[#d4c9a8] shadow-sm">
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4">
        ...
        <nav className="hidden md:flex items-center gap-1">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="text-base font-semibold text-[#3a3a2e] hover:text-[#3d6b35] hover:bg-[#eef5ea] px-4 py-2.5 rounded-xl transition-colors"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="hidden md:flex items-center gap-2 bg-white border-2 border-[#d4c9a8] rounded-xl px-4 py-2.5 w-56 focus-within:border-[#3d6b35] transition-colors">
            <Search size={18} className="text-[#7a9e5f] shrink-0" />
            <input
              type="text"
          ...
          >
            {menuOpen
              ? <X size={24} className="text-[#3d6b35]" />
              : <Menu size={24} className="text-[#3d6b35]" />
            }
          </button>
        </div>
      </div>
      ...
      {menuOpen && (
        <div className="md:hidden bg-[#faf7f2] border-t-2 border-[#d4c9a8] px-4 py-4 flex flex-col gap-2">
          <div className="flex items-center gap-2 bg-white border-2 border-[#d4c9a8] rounded-xl px-4 py-3 mb-2">
            <Search size={20} className="text-[#7a9e5f] shrink-0" />
            <input
              type="text"
              placeholder="Search products..."
              className="bg-transparent outline-none text-base w-full text-[#3a3a2e] placeholder:text-[#b0a890]"
          ...
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="flex items-center px-4 py-4 text-lg font-semibold text-[#3a3a2e] hover:bg-[#eef5ea] hover:text-[#3d6b35] rounded-xl transition-colors"
